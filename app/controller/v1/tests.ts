...
console.log("Agent 表创建");
        const phone = '188' + Math.ceil(Math.random() * 100000000);
        let agent: any = new this.ctx.model.User({
...
console.log("生成测试数据");
    ...
    //创建馆长
    async createCurator(){
       let users = await this.ctx.model.User.update(
            {"_id":new mongoose.Types.ObjectId(this.ctx.query.userId)},
            {
                $set:{
                    "mobileAuth.auth":true,
