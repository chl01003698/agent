...
var fs = require('fs');
class AgentsController extends egg_1.Controller {
    //用户登录
    async login() {
        let result = {};
        ...
        if (error != null && error.name == "ValidationError") {
            result = reply_1.default.err("手机号应为11位,密码不能应小于6位");
        }
        else {
    ...
    async logout() {
        ...
        this.ctx.body = reply_1.default.success(true, "退出成功");
    }
    //我的棋牌室
    async myChessRoom() {
    ...
    async editPassword() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("密码长度不能小于6位");
        }
        else {
            if (request.password == request.checkpassword) {
                ...
                this.ctx.body = reply_1.default.err("新密码输入不一致");
            }
        }
    }
    //修改棋牌室名称
    async myChessNameEditor() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("棋牌室名称只能输入1~8位字符");
        }
        else {
    ...
    async myChessDeclarationEditor() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("宣言不能小于1位");
        }
        else {
    ...
    async wechatNumberEditor() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("微信号不能小于1位");
        }
        else {
    ...
    async untyingUsers() {
        ...
        if (error != null && error.name == "ValidationError") {
            result = reply_1.default.err("解绑字段不能小于1位");
        }
        else {
    ...
    async switchIdentity() {
        ...
        let result = {};
        if (error != null && error.name == "ValidationError") {
            result = reply_1.default.err("切换身份字段不能小于1位");
        }
        else {
            //身份在列表中.
            const arr = ['curator', 'agentSuper', 'agent'];
            result = reply_1.default.err('身份切换失败');
            let flag = false;
            ...
            if (flag) {
                ...
                if (this.ctx.session.identity.current != identity) {
                    ...
                    result = reply_1.default.err("当前身份不需要切换");
                }
            }
        }
        this.ctx.body = result;
    ...
    async realAuthRealname() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("真实姓名和身份证号不能小于1位");
        }
        else {
    ...
    async realAuthRealCity() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("请选择省市信息");
        }
        else {
    ...
    async getUserCharts() {
        ...
        if (error != null && error.name == "ValidationError") {
            result = reply_1.default.err("请选择查看时间范围");
        }
        else {
