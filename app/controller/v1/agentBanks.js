...
class AgentBanksController extends egg_1.Controller {
    //获取用户银行信息
    async index() {
    ...
    async create() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("请仔细检查录入信息");
        }
        else {
            ...
            if (auth) {
                ...
                this.ctx.body = reply_1.default.err('验证码错误请重试');
            }
        }
    }
    //用户申请提现
    async withdrawals() {
        ...
        if (error != null && error.name == "ValidationError") {
            result = reply_1.default.err("输入有误,请检查金额,手机号,验证码");
        }
        else {
    ...
    async verificationCode() {
        ...
        if (error != null && error.name == "ValidationError") {
            this.ctx.body = reply_1.default.err("输入有误,请检查手机号");
        }
        else {
            ...
            if (validatePhone) {
                ...
                if (result.result) {
                    this.ctx.body = reply_1.default.success({ smscode: authCode }, "验证码发送成功");
                }
                else {
                    this.ctx.body = reply_1.default.err(result.sendResult.detail, result.sendResult.code);
                }
            }
            else {
